	...
	if(isset($_POST['add']))
	{
	  ...
	  if($num_row !=0)
	  {
	    ...
	    foreach($rows as $row)
	    {
	      ...
	      if ($user2ID != $id)
	      {	
		echo "<span style='display: inline-block; padding: 10px;'>";  
		echo "<form name='chat' method='post' action='contacts.php' style='display: inline-block; padding: 10px;'>"; 
		echo "<button type='submit' name='chat' value='".$row2['id']."'> <image style='width:14px;height:14px;padding:10px;' src='http://www.famfamfam.com/lab/icons/mini/icons/comment.gif'/> </button>"; ; 
		echo "</form>";
